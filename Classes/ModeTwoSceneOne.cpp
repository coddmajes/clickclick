...
Scene* ModeTwoSceneOne ::createScene()
{
    auto scene = Scene::create();
    auto layer = ModeTwoSceneOne::create();
    scene->addChild(layer);
    return scene;
}

bool ModeTwoSceneOne::init()
{
...
void ModeTwoSceneOne::setUi()
{
    ...
    btnSignOut->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne::btnSignOutCallback, this));
    ...
    btnPause->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne::btnPauseCallback, this));
    
    //提示
    auto labelSign = Label::createWithSystemFont("前方高能！当前关卡需要答对8题才能过关哦！", "", 24);
    ...
    if(m_limit==0)
    {
        ...
        schedule(schedule_selector(ModeTwoSceneOne::limitTime), 1.0f); //自定义调度器
...
void ModeTwoSceneOne::btnWay()
{
    ...
    for (int j=0; j<2; j++)
    {
        ...
        if(j==0)
        {
            ...
            btnModeOne->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne::btnModeTwoCallFunc, this,m_resultsOne, x,y));
        }
        if(j==1)
        {
            switch (result1[j])
            ...
            btnModeOne->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne::btnModeTwoCallFunc, this,m_resultsTwo, x,y));
        }
...
void ModeTwoSceneOne::btnModeTwoCallFunc(Ref *target, int tag,int x, int y)
{
    auto visibleSize = Director::getInstance()->getVisibleSize();
    if(m_tag==tag)
    {
...
void ModeTwoSceneOne::limitTime(float time)
{
    m_limitTime--;
    auto visibleSize = Director::getInstance()->getVisibleSize();
    ...
    if(m_limitTime<=10&&m_limitTime>=0)
    {
        ...
        if(m_limitTime==0&&score>=8)
        {
            this->changeScene();
        }
        else if(m_limitTime==0&&score<8)
        {
            ...
            gameHome->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne::btnGameHomeCallback, this));
            ...
            gameAgain->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne::btnGameAgainCallback, this));
...
void ModeTwoSceneOne::changeScene()
{
    auto sceneModeTwo1 = ModeTwoSceneOne1::createScene();
    auto fadeInModeTwo1 = TransitionCrossFade::create(1.5, sceneModeTwo1);
    Director::getInstance()->pushScene(fadeInModeTwo1);
}

void ModeTwoSceneOne::btnSignOutCallback(Ref *target)
{
    CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
...
void ModeTwoSceneOne::btnPauseCallback(Ref *target)
{
...
void ModeTwoSceneOne::btnGameHomeCallback(Ref *target)
{
...
void ModeTwoSceneOne::btnGameAgainCallback(Ref *target)
{
...
void ModeTwoSceneOne::onEnterTransitionDidFinish()
{
