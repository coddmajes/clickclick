...
Scene* ModeTwoSceneThree1 ::createScene()
{
    auto scene = Scene::create();
    auto layer = ModeTwoSceneThree1::create();
    scene->addChild(layer);
    return scene;
}

bool ModeTwoSceneThree1::init()
{
    if ( !LayerColor::init() )
    {
        return false;
...
void ModeTwoSceneThree1::setUi()
{
    ...
    //提示
    auto labelSign = Label::createWithSystemFont("前方高能！当前关卡需要答对200题才能过关哦！", "", 24);
    ...
    btnSignOut->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree1::btnSignOutCallback, this));
    ...
    btnPause->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree1::btnPauseCallback, this));
    ...
    if(m_limit==0)
    {
        ...
        schedule(schedule_selector(ModeTwoSceneThree1::limitTime), 1.0f); //自定义调度器
...
void ModeTwoSceneThree1::btnWay()
{
    auto visibleSize = Director::getInstance()->getVisibleSize();
    //设置button以及随机数
    int result[20];
    ...
    for (int j=0; j<2; j++)
    {
        ...
        if(j==0)
        {
            ...
            btnModeOne->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree1::btnModeTwoCallFunc, this,m_resultsOne, x, y));
        }
        if(j==1)
        {
            switch (result1[j])
            ...
            btnModeOne->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree1::btnModeTwoCallFunc, this,m_resultsTwo, x, y));
        }
...
void ModeTwoSceneThree1::btnModeTwoCallFunc(Ref *target, int tag,int x, int y)
{
    auto visibleSize = Director::getInstance()->getVisibleSize();
    if(m_tag==tag)
    {
...
void ModeTwoSceneThree1::limitTime(float time)
{
    m_limitTime--;
    auto visibleSize = Director::getInstance()->getVisibleSize();
    m_labelTime->setString(StringUtils::format("%dS",m_limitTime));
    if(m_limitTime<=60&&m_limitTime>=0)
    {
        ...
        if(m_limitTime==0&&score>=200)
        {
            this->changeScene();
        }
        else if(m_limitTime==0&&score<200)
        {
            ...
            gameHome->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree1::btnGameHomeCallback, this));
            ...
            gameHome->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree1::btnGameAgainCallback, this));
        }
        userDefault->setIntegerForKey("score", score);
    }
}

void ModeTwoSceneThree1::btnSignOutCallback(Ref *target)
{
    CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
...
void ModeTwoSceneThree1::btnPauseCallback(Ref *target)
{
...
void ModeTwoSceneThree1::btnGameHomeCallback(Ref *target)
{
...
void ModeTwoSceneThree1::btnGameAgainCallback(Ref *target)
{
...
void ModeTwoSceneThree1::changeScene()
{
    CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
    auto layer =ModeTwoLastScore ::create();
}