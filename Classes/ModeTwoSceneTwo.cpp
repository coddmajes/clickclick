...
Scene* ModeTwoSceneTwo ::createScene()
{
    auto scene = Scene::create();
    auto layer = ModeTwoSceneTwo::create();
    scene->addChild(layer);
    return scene;
}

bool ModeTwoSceneTwo::init()
{
    if ( !LayerColor::init() )
    {
        return false;
...
void ModeTwoSceneTwo::setUi()
{
    ...
    //提示
    auto labelSign = Label::createWithSystemFont("前方高能！当前关卡需要答对60题才能过关哦！", "", 24);
    ...
    btnSignOut->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneTwo::btnSignOutCallback, this));
    ...
    btnPause->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneTwo::btnPauseCallback, this));
    ...
    if(m_limit==0)
    {
        ...
        schedule(schedule_selector(ModeTwoSceneTwo::limitTime), 1.0f); //自定义调度器
...
void ModeTwoSceneTwo::btnWay()
{
    ...
    for (int j=0; j<2; j++)
    {
        ...
        if(j==0)
        {
            ...
            btnModeOne->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneTwo::btnModeTwoCallFunc, this,m_resultsOne, x,y));
        }
        if(j==1)
        {
            switch (result1[j])
            ...
            btnModeOne->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneTwo::btnModeTwoCallFunc, this,m_resultsTwo, x,y));
        }
...
void ModeTwoSceneTwo::btnModeTwoCallFunc(Ref *target, int tag,int x,int y)
{
    auto visibleSize = Director::getInstance()->getVisibleSize();
    if(m_tag==tag)
    {
...
void ModeTwoSceneTwo::limitTime(float time)
{
    m_limitTime--;
    auto visibleSize = Director::getInstance()->getVisibleSize();
    m_labelTime->setString(StringUtils::format("%dS",m_limitTime));
    if(m_limitTime<=30&&m_limitTime>=0)
    {
        ...
        if(m_limitTime==0&&score>=60)
        {
            this->changeScene();
        }
        else if(m_limitTime==0&&score<60)
        {
            ...
            gameHome->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneTwo::btnGameHomeCallback, this));
            ...
            gameAgain->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneTwo::btnGameAgainCallback, this));
...
void ModeTwoSceneTwo::changeScene()
{
    auto sceneModeTwo2 = ModeTwoSceneTwo1::createScene();
    auto fadeInModeTwo2 = TransitionCrossFade::create(1.5, sceneModeTwo2);
    Director::getInstance()->pushScene(fadeInModeTwo2);
}

void ModeTwoSceneTwo::btnSignOutCallback(Ref *target)
{
    CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
...
void ModeTwoSceneTwo::btnPauseCallback(Ref *target)
{
...
void ModeTwoSceneTwo::btnGameHomeCallback(Ref *target)
{
...
void ModeTwoSceneTwo::btnGameAgainCallback(Ref *target)
{
