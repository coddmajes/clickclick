...
Scene* ModeTwoSceneOne1 ::createScene()
{
    auto scene = Scene::create();
    auto layer = ModeTwoSceneOne1::create();
    scene->addChild(layer);
    return scene;
}

bool ModeTwoSceneOne1::init()
{
...
void ModeTwoSceneOne1::setUi()
{
    ...
    auto spriteModeTwo = Sprite::create("mode2-1background.png");
    ...
    //提示
    auto labelSign = Label::createWithSystemFont("前方高能！当前关卡需要答对28题才能过关哦！", "", 24);
    ...
    btnSignOut->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne1::btnSignOutCallback, this));
    ...
    btnPause->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne1::btnPauseCallback, this));
    ...
    if(m_limit==0)
    {
        ...
        schedule(schedule_selector(ModeTwoSceneOne1::limitTime), 1.0f); //自定义调度器
...
void ModeTwoSceneOne1::btnWay()
{
    ...
    for (int j=0; j<2; j++)
    {
        ...
        if(j==0)
        {
            ...
            btnModeOne->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne1::btnModeTwoCallFunc, this,m_resultsOne, x,y));
        }
        if(j==1)
        {
            switch (result1[j])
            ...
            btnModeOne->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne1::btnModeTwoCallFunc, this,m_resultsTwo, x,y));
        }
...
void ModeTwoSceneOne1::btnModeTwoCallFunc(Ref *target, int tag,int x, int y)
{
    auto visibleSize = Director::getInstance()->getVisibleSize();
    if(m_tag==tag)
    {
    ...
    {
        auto missOne = Sprite::create("miss.jpg");
...
void ModeTwoSceneOne1::limitTime(float time)
{
    m_limitTime--;
    auto visibleSize = Director::getInstance()->getVisibleSize();
    m_labelTime->setString(StringUtils::format("%dS",m_limitTime));
    if(m_limitTime<=20&&m_limitTime>=0)
    {
        ...
        if(m_limitTime==0&&score>=28)
        {
            this->changeScene();
        }
        else if(m_limitTime==0&&score<28)
        {
            ...
            gameHome->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne1::btnGameHomeCallback, this));
            ...
            gameAgain->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneOne1::btnGameAgainCallback, this));
...
void ModeTwoSceneOne1::changeScene()
{
    auto sceneModeTwo1 = ModeTwoSceneTwo::createScene();
    auto fadeInModeTwo1 = TransitionCrossFade::create(1.5, sceneModeTwo1);
    Director::getInstance()->pushScene(fadeInModeTwo1);
}

void ModeTwoSceneOne1::btnSignOutCallback(Ref *target)
{
    CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
...
void ModeTwoSceneOne1::btnPauseCallback(Ref *target)
{
...
void ModeTwoSceneOne1::btnGameHomeCallback(Ref *target)
{
...
void ModeTwoSceneOne1::btnGameAgainCallback(Ref *target)
{
