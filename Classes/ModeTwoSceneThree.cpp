...
Scene* ModeTwoSceneThree ::createScene()
{
    auto scene = Scene::create();
    auto layer = ModeTwoSceneThree::create();
    scene->addChild(layer);
    return scene;
}

bool ModeTwoSceneThree::init()
{
    if ( !LayerColor::init() )
    {
        return false;
...
void ModeTwoSceneThree::setUi()
{
    ...
    auto spriteModeTwo = Sprite::create("mode2-5background.png");
    ...
    //提示
    auto labelSign = Label::createWithSystemFont("前方高能！当前关卡需要答对140题才能过关哦！", "", 24);
    ...
    //当前关卡
    auto labellevel = Label::createWithTTF(StringUtils::format("%d", 5), "fonts/Marker Felt.ttf", 36);
    ...
    btnSignOut->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree::btnSignOutCallback, this));
    ...
    btnPause->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree::btnPauseCallback, this));
    ...
    if(m_limit==0)
    {
        ...
        schedule(schedule_selector(ModeTwoSceneThree::limitTime), 1.0f); //自定义调度器
...
void ModeTwoSceneThree::btnWay()
{
    ...
    for (int j=0; j<2; j++)
    {
        ...
        auto btnModeOne = Button::create("buttonmode2-5.png");
        ...
        if(j==0)
        {
            ...
            btnModeOne->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree::btnModeTwoCallFunc, this,m_resultsOne, x, y));
        }
        if(j==1)
        {
            switch (result1[j])
            ...
            btnModeOne->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree::btnModeTwoCallFunc, this,m_resultsTwo, x, y));
        }
...
void ModeTwoSceneThree::btnModeTwoCallFunc(Ref *target, int tag,int x, int y)
{
    auto visibleSize = Director::getInstance()->getVisibleSize();
    if(m_tag==tag)
    {
...
void ModeTwoSceneThree::limitTime(float time)
{
    m_limitTime--;
    auto visibleSize = Director::getInstance()->getVisibleSize();
    m_labelTime->setString(StringUtils::format("%dS",m_limitTime));
    if(m_limitTime<=50&&m_limitTime>=0)
    {
        ...
        if(m_limitTime==0&&score>=140)
        {
            this->changeScene();
        }
        else if(m_limitTime==0&&score<140)
        {
            ...
            gameHome->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree::btnGameHomeCallback, this));
            ...
            gameAgain->addClickEventListener(CC_CALLBACK_1(ModeTwoSceneThree::btnGameAgainCallback, this));
...
void ModeTwoSceneThree::btnSignOutCallback(Ref *target)
{
    CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
...
void ModeTwoSceneThree::btnPauseCallback(Ref *target)
{
...
void ModeTwoSceneThree::btnGameHomeCallback(Ref *target)
{
...
void ModeTwoSceneThree::btnGameAgainCallback(Ref *target)
{
...
void ModeTwoSceneThree::changeScene()
{
    auto sceneModeTwo3 = ModeTwoSceneThree1::createScene();
    auto fadeInModeTwo3 = TransitionCrossFade::create(1.5, sceneModeTwo3);
    Director::getInstance()->pushScene(fadeInModeTwo3);
}